	...
	<div class="panel-body">
		<form action="{{ route('admin.forms.update', $form->slug) }}" method="post">
			...
			<div class="form-group">
				<div class="checkbox">
					<label for="accept_submissions">
						<input type="checkbox" name="accept_submissions" id="accept_submissions" {!! $form->accept_submissions ? 'checked' : '' !!}>
			...
			<div class="form-group">
				...
				<input class="form-control" type="text" name="send_email_to" id="send_email_to" value="{{ $form->send_email_to }}">
			...
			<div class="form-group">
				...
				<input class="form-control" type="text" name="confirmation_email_field" id="confirmation_email_field" value="{{ $form->confirmation_email_field }}">
			...
			<div class="form-group">
				...
				<input class="form-control" type="url" name="redirect_to_url" id="redirect_to_url" value="{{ $form->redirect_to_url }}">
			</div>

			<div class="form-group">
				<label for="owner_email">{{ trans('form.label.owner_email') }}</label>
				<input class="form-control" type="email" name="owner_email" id="owner_email" value="{{ $form->owner_email }}">
				<span class="help-block">{{ trans('form.help.owner_email') }}</span>
			</div>

			<div class="form-group">
				<label for="owner_name">{{ trans('form.label.owner_name') }}</label>
				<input class="form-control" type="text" name="owner_name" id="owner_name" value="{{ $form->owner_name }}">
		...
		<form action="{{ route('admin.forms.destroy', $form->slug) }}" method="post">
			...
			<input class="btn btn-danger" type="submit" value="{{ trans('form.action.delete') }}"
				   onclick="return confirm('{{ trans('admin.sure_to_delete', ['item' => $form->title]) }}') ? true : false">
		</form>
	</div>
</div>
...
<div class="row">
	<div class="col-md-6">
		...
		<div class="panel panel-default">
			<div class="panel-body">
				<form action="{{ route('admin.forms.fields.update', [$form->slug, $field->slug]) }}" method="post">
					{!! method_field('put') !!}
					{!! csrf_field() !!}

					<div class="form-group">
						<label for="slug">{{ trans('field.label.slug') }}</label>
						<input class="form-control" type="text" name="slug" id="slug" value="{{ $field->slug }}" required>
					</div>

					<div class="form-group">
						<label for="title">{{ trans('field.label.title') }}</label>
						<input class="form-control" type="text" name="title" id="title" value="{{ $field->title }}" required>
					</div>

					<div class="form-group">
						<label for="rules">{{ trans('field.label.rules') }}</label>
						<input class="form-control" type="text" name="rules" id="rules" value="{{ $field->rules }}">
						<span class="help-block">{{ trans('field.help.rules') }}</span>
					</div>

					<input class="btn btn-primary" type="submit" value="{{ trans('field.action.save') }}">
				</form>
				<form action="{{ route('admin.forms.fields.destroy', [$form->slug, $field->slug]) }}" method="post">
					{!! method_field('delete') !!}
					{!! csrf_field() !!}

					<input class="btn btn-danger" type="submit" value="{{ trans('field.action.delete') }}"
						   onclick="return confirm('{{ trans('admin.sure_to_delete', ['item' => $field->title]) }}') ? true : false">
				</form>
			</div>
		</div>
		@empty
