...
<table class="table table-striped">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<a class="btn btn-default" href="{{ route('admin.forms.submissions.show', [$form->slug, $submission->id]) }}">{{ trans('submission.action.show') }}</a>
				<form action="{{ route('admin.forms.submissions.show', [$form->slug, $submission->id]) }}" method="post">
					...
					<input class="btn btn-danger" type="submit" value="{{ trans('submission.action.delete') }}"
						   onclick="return confirm('{{ trans('admin.sure_to_delete', ['item' => $submission->created_at]) }}') ? true : false">
				</form>
			</td>
		</tr>
		@empty
